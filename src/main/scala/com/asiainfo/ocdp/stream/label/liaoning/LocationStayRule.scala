...
class LocationStayRule extends Label {
  ...
  override def attachLabel(line: Map[String, String], cache: StreamingCache, labelQryData: mutable.Map[String, mutable.Map[String, String]]): (Map[String, String], StreamingCache) = {
    ...
    println("---------开始执行驻留时长标签操作---------")
    val cacheInstance = if (cache == null) new StayLabelProps else cache.asInstanceOf[StayLabelProps]

    // cache中各区域的属性map
    val cacheImmutableMap = cacheInstance.stayLabelPropList
    // map属性转换
    ...
    //3更新缓存
    // map属性转换
    cacheInstance.stayLabelPropList = transformCacheMap2ImmutableMap(cacheMutableMap)
...
class StayLabelProps() extends StreamingCache with scala.Serializable {
  var stayLabelPropList = Map[String, Map[String, String]]()
}